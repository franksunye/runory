        ...
        <div className="border-t border-slate-200 bg-white px-6 py-5 md:hidden">
          <nav className="mx-auto grid max-w-7xl gap-1" aria-label={t("common.mobileNavigation")}>
            ...
            <div className="my-2 flex gap-2 px-3">
              {(["en", "zh"] as const).map((item) => <button key={item} type="button" onClick={() => setLocale(item)} className={`rounded-lg px-3 py-2 text-xs font-bold ${locale === item ? "bg-slate-950 text-white" : "bg-slate-100 text-slate-600"}`}>{item === "en" ? "English" : "中文"}</button>)}
            </div>
