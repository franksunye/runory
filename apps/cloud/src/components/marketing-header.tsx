...
export function MarketingHeader({ authenticated = false }: { authenticated?: boolean }) {
  ...
  return (
    <header className="relative z-30 border-b border-black/10 bg-[#fbf8f1]/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[68px] max-w-7xl items-center justify-between px-5 sm:px-6 lg:h-[72px] lg:px-10">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <div className="flex items-center rounded-lg border border-black/10 bg-white p-0.5">
            {(["en", "zh"] as const).map((item) => <button key={item} type="button" onClick={() => setLocale(item)} className={`rounded-md px-2 py-1.5 text-xs font-bold ${locale === item ? "bg-neutral-950 text-white" : "text-neutral-500"}`}>{item === "en" ? "EN" : "中文"}</button>)}
          </div>
      ...
      {open && <div className="border-t border-black/10 bg-[#fbf8f1] px-5 py-5 md:hidden"><nav className="mx-auto grid max-w-7xl gap-1">{links.map((link) => <Link key={link.href} href={link.href} onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 text-base font-semibold text-neutral-800 hover:bg-black/5">{link.label}</Link>)}<div className="mt-3 flex gap-2 px-3">{(["en", "zh"] as const).map((item) => <button key={item} type="button" onClick={() => setLocale(item)} className={`rounded-lg px-3 py-2 text-xs font-bold ${locale === item ? "bg-neutral-950 text-white" : "border border-black/10 bg-white text-neutral-600"}`}>{item === "en" ? "English" : "中文"}</button>)}</div><Link href={authenticated ? "/dashboard" : "/pilot"} onClick={() => setOpen(false)} className="mt-4 inline-flex min-h-12 items-center justify-center rounded-full bg-neutral-950 px-5 text-sm font-semibold text-white">{authenticated ? copy.workspace : copy.pilot}</Link></nav></div>}
    </header>
  );
}
