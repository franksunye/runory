...
export async function AgentRuntimeArticlePage({ slug }: { slug: string }) {
  const cookieStore = await cookies();
  const locale = cookieStore.get(LOCALE_COOKIE)?.value === "zh" ? "zh" : "en";
  const article = getAgentRuntimeArticle(locale, slug);
    ...
    <main className="min-h-screen bg-[#fbf8f1] text-neutral-950">
      ...
      <article>
        <header className="mx-auto max-w-5xl px-5 pb-14 pt-12 sm:px-6 sm:pb-20 sm:pt-20 lg:px-10">
          <Link href="/resources" className="inline-flex items-center gap-2 text-sm font-semibold text-neutral-600 hover:text-neutral-950"><ArrowLeft size={16} />{locale === "zh" ? "返回资源" : "Back to Resources"}</Link>
          ...
          <div className="mx-auto max-w-5xl px-5 py-14 sm:px-6 sm:py-20 lg:px-10">
            <div className="grid gap-12 lg:grid-cols-[1fr_260px] lg:gap-16">
              ...
              <aside className="h-fit rounded-2xl border border-black/10 bg-[#fbf8f1] p-6 lg:sticky lg:top-24">
                <p className="text-xs font-semibold uppercase tracking-[.16em] text-orange-600">{locale === "zh" ? "关键结论" : "Key takeaways"}</p>
        ...
        <section className="mx-auto max-w-5xl px-5 py-16 sm:px-6 sm:py-20 lg:px-10">
          <p className="text-sm font-semibold uppercase tracking-[.18em] text-orange-600">{locale === "zh" ? "继续阅读" : "Continue reading"}</p>
          <div className="mt-7 grid gap-4 md:grid-cols-3">{article.relatedSlugs.filter((relatedSlug) => getAgentRuntimeArticle(locale, relatedSlug)).map((relatedSlug) => { const related = getAgentRuntimeArticle(locale, relatedSlug)!; return <Link key={relatedSlug} href={`/resources/${relatedSlug}`} className="group rounded-2xl border border-black/10 bg-white p-6"><span className="text-xs font-semibold uppercase tracking-[.14em] text-orange-600">{related.category}</span><h2 className="mt-4 text-lg font-semibold leading-7">{related.title}</h2><span className="mt-6 inline-flex items-center gap-2 text-sm font-semibold">{locale === "zh" ? "阅读文章" : "Read article"}<ArrowRight size={15} className="transition group-hover:translate-x-1" /></span></Link>; })}</div>
        </section>

        <section className="mx-auto max-w-7xl px-5 pb-20 sm:px-6 sm:pb-28 lg:px-10"><div className="rounded-[28px] bg-neutral-950 px-6 py-10 text-white sm:px-10 sm:py-14 lg:flex lg:items-end lg:justify-between lg:gap-12"><div><p className="text-sm font-semibold uppercase tracking-[.18em] text-orange-300">{locale === "zh" ? "在真实流程中验证" : "Prove it in a real workflow"}</p><h2 className="mt-4 max-w-3xl font-serif text-4xl tracking-[-.035em] sm:text-5xl">{locale === "zh" ? "从一条关键业务流程开始。" : "Start with one priority business workflow."}</h2><p className="mt-5 max-w-2xl leading-7 text-neutral-300">{locale === "zh" ? "在 1–2 周内验证 Agent、业务数据、权限、流程与执行闭环。" : "Validate the Agent, business data, permissions, workflow, and execution loop in 1–2 weeks."}</p></div><Link href="/pilot" className="mt-8 inline-flex min-h-12 shrink-0 items-center gap-2 rounded-full bg-white px-6 font-semibold text-neutral-950 lg:mt-0">{locale === "zh" ? "启动试点" : "Start a Pilot"}<ArrowRight size={18} /></Link></div></section>
      </article>
      <MarketingFooter />
    </main>
  );
