...
export default function WaterproofingSolutionPage() {
  ...
  return <main className="min-h-screen bg-[#fbf8f1] text-neutral-950"><MarketingHeader />
    ...
    <section className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 px-5 py-14 sm:px-6 sm:py-20 md:flex-row md:items-center lg:px-10"><div><p className="text-sm font-semibold uppercase tracking-[.18em] text-orange-600">{copy.pilotLabel}</p><h2 className="mt-4 max-w-3xl font-serif text-3xl leading-tight tracking-[-.03em] sm:text-4xl">{copy.pilotTitle}</h2><p className="mt-3 max-w-2xl leading-7 text-neutral-600">{copy.pilotBody}</p></div><Link href="/pilot" aria-label={copy.pilotCta} className="inline-flex min-h-12 shrink-0 items-center gap-2 rounded-full bg-neutral-950 px-6 py-3 font-semibold text-white">{copy.pilotCta} <ArrowRight size={18} /></Link></section>
