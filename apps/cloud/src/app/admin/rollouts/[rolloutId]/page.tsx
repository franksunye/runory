  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 text-sm text-slate-500">
        ...
        <Link href={`/admin/releases/${rollout.catalogReleaseId}`} className="hover:text-slate-700">发布</Link>
        <ChevronRight size={14} />
      ...
      <div className="mt-4 flex items-center gap-3">
        <h1 className="text-2xl font-bold tracking-tight text-slate-950">Rollout 详情</h1>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <h3 className="text-sm font-bold text-slate-900">Rollout 元数据</h3>
          <dl className="mt-3 space-y-2 text-sm">
            <MetaRow label="Rollout ID" value={rollout.id} mono />
            <MetaRow label="Release ID" value={rollout.catalogReleaseId} mono />
            <MetaRow label="目标类型" value={rollout.targetType} />
            <MetaRow label="状态" value={badge.label} />
            <MetaRow label="成功阈值" value={`${(rollout.successThreshold * 100).toFixed(0)}%`} />
            <MetaRow label="失败阈值" value={`${(rollout.failureThreshold * 100).toFixed(0)}%`} />
            {rollout.startedBy && <MetaRow label="启动者" value={rollout.startedBy} mono />}
            <MetaRow label="启动时间" value={formatDateTime(rollout.startedAt)} />
            <MetaRow label="完成时间" value={formatDateTime(rollout.completedAt)} />
            <MetaRow label="创建时间" value={formatDateTime(rollout.createdAt)} />
          </dl>
        </div>

        {/* Progress */}
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <h3 className="text-sm font-bold text-slate-900">进度</h3>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            <ProgressStat label="总数" value={progress.total} color="text-slate-900" />
            <ProgressStat label="成功" value={progress.succeeded} color="text-emerald-700" />
            <ProgressStat label="失败" value={progress.failed} color="text-red-700" />
            <ProgressStat label="运行中" value={progress.running} color="text-blue-700" />
            <ProgressStat label="待处理" value={progress.pending} color="text-slate-600" />
            <ProgressStat label="跳过" value={progress.skipped} color="text-slate-400" />
          </div>

          {/* Rates */}
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div className="rounded-lg bg-emerald-50 p-3">
              <p className="text-xs font-semibold text-slate-500">成功率</p>
              <p className="mt-1 text-lg font-bold text-emerald-700">{(progress.successRate * 100).toFixed(1)}%</p>
            </div>
            <div className="rounded-lg bg-red-50 p-3">
              <p className="text-xs font-semibold text-slate-500">失败率</p>
      ...
      {/* Target list */}
      <div className="mt-8">
        <h2 className="text-lg font-bold text-slate-950">目标列表</h2>
        {targets.length === 0 ? (
          <p className="mt-3 text-sm text-slate-500">暂无目标。</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-slate-200 bg-slate-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-slate-600">From Version</th>
                  <th className="px-4 py-3 text-left font-semibold text-slate-600">To Version</th>
                  <th className="px-4 py-3 text-left font-semibold text-slate-600">原因</th>
                  <th className="px-4 py-3 text-left font-semibold text-slate-600">开始时间</th>
                  <th className="px-4 py-3 text-left font-semibold text-slate-600">完成时间</th>
                </tr>
              </thead>
