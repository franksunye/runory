...
const VIEW_KEY = "question_map_form";

export default function QuestionMapDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm("确定要删除此问题地图吗？此操作不可撤销。")) return;
    setDeleting(true);
    setError(null);
    try {
      const res = await fetch(
      ...
      if (json.success) {
        notifyWorkspaceDataChanged();
        router.push(`/w/${workspaceId}/question-maps`);
        router.refresh();
      } else {
  ...
  if (!record) {
    return (
      <div className="space-y-4">
        ...
        <Link
          href={`/w/${workspaceId}/question-maps`}
