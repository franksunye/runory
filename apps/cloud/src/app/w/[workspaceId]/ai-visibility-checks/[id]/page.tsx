...
const VIEW_KEY = "ai_visibility_check_form";

export default function AIVisibilityCheckDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm("确定要删除此可见性检查吗？此操作不可撤销。")) return;
    setDeleting(true);
    setError(null);
    try {
      const res = await fetch(
      ...
      if (json.success) {
        notifyWorkspaceDataChanged();
        router.push(`/w/${workspaceId}/ai-visibility-checks`);
        router.refresh();
      } else {
  ...
  if (!record) {
    return (
      <div className="space-y-4">
        ...
        <Link
          href={`/w/${workspaceId}/ai-visibility-checks`}
