...
const VIEW_KEY = "quote_approval_form";

export default function QuoteApprovalDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm("确定要删除此报价审批吗？此操作不可撤销。")) return;
    setDeleting(true);
    setError(null);
    try {
      const res = await fetch(
      ...
      if (json.success) {
        notifyWorkspaceDataChanged();
        router.push(`/w/${workspaceId}/quote-approvals`);
        router.refresh();
      } else {
  ...
  if (!record) {
    return (
      <div className="space-y-4">
        ...
        <Link
          href={`/w/${workspaceId}/quote-approvals`}
