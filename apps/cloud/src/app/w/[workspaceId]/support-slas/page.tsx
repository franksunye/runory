...
const OBJECT_KEY = "support_sla";
const VIEW_KEY = "support_sla_list";
const PAGE_SIZE = 20;
...
export default function SupportSlaListPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">SLA 策略</h1>
          <p className="mt-1 text-sm text-slate-500">管理服务等级协议</p>
        </div>
        {hasPack && (
          <button
            type="button"
            onClick={() =>
              router.push(`/w/${workspaceId}/support-slas/new`)
            }
      ...
      {!hasPack ? (
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-8 text-center">
          ...
          <p className="mt-1 text-sm text-blue-700">安装 CRM Lite Pack 后即可开始管理 SLA 策略。</p>
          <Link
        ...
        <div className="space-y-3">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-purple-800">
                    ...
                    {" "}已加入 SLA 策略列表和表单。此提示仅在字段变更后出现一次。
                  </p>
                </div>
                <button
                  type="button"
          ...
          {totalCount === 0 ? (
            isSearching ? (
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-8 text-center">
                <p className="text-sm text-slate-500">还没有 SLA 策略</p>
                <button
                  type="button"
                  onClick={() => router.push(`/w/${workspaceId}/support-slas/new`)}
            ...
            <>
              <SchemaTable
                ...
                basePath={`/w/${workspaceId}/support-slas`}
              />
              {hasMore && (
