...
const VIEW_KEY = "support_sla_form";

export default function SupportSlaDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm("确定要删除此 SLA 策略吗？此操作不可撤销。")) return;
    setDeleting(true);
    setError(null);
    try {
      const res = await fetch(
      ...
      if (json.success) {
        notifyWorkspaceDataChanged();
        router.push(`/w/${workspaceId}/support-slas`);
        router.refresh();
      } else {
  ...
  if (!record) {
    return (
      <div className="space-y-4">
        ...
        <Link
          href={`/w/${workspaceId}/support-slas`}
