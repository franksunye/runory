...
const VIEW_KEY = "product_service_form";
...
export default function ProductServiceDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm("确定要删除此产品/服务吗？此操作不可撤销。")) return;
    setDeleting(true);
    setError(null);
    try {
      const res = await fetch(
      ...
      if (json.success) {
        notifyWorkspaceDataChanged();
        router.push(`/w/${workspaceId}/products-services`);
        router.refresh();
      } else {
  ...
  if (!record) {
    return (
      <div className="space-y-4">
        ...
        <Link
          href={`/w/${workspaceId}/products-services`}
