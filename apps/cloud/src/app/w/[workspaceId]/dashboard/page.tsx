...
import { Activity, ArrowRight, Boxes, CheckCircle2, CheckSquare, Clock3, Database, PackagePlus, Puzzle, RefreshCw, ShieldCheck, Sparkles } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const { data: customerRecords = [], isLoading: loadingCustomers } = useRecords(workspaceId, "customer");
  const loading = loadingInst && loadingExt && loadingAudit && loadingTasks && loadingCustomers;
  ...
  const handleSeedDemo = async () => {
    ...
    try {
      ...
      await refreshAll(); notifyWorkspaceDataChanged(); router.refresh();
  ...
  return <div className="space-y-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="app-eyebrow">Workbench</p><h1 className="mt-2 text-3xl font-bold tracking-[-.025em] text-slate-950">工作台</h1><p className="mt-2 text-sm text-slate-500">今天需要关注什么？管理已安装能力、扩展状态与最近活动。</p></div>
      ...
      <div className="flex flex-col justify-between gap-7 lg:flex-row lg:items-center">
        ...
        {hasPack ? (
          customerRecords.length === 0 ? (
            <button onClick={handleSeedDemo} disabled={seeding} className="app-button-primary min-w-fit"><Database size={18} />{seeding ? "正在加载..." : "加载示例数据"}</button>
          ) : (
            <div className="flex min-w-fit items-center gap-3 rounded-xl border border-emerald-200 bg-white/80 px-4 py-3 text-sm font-semibold text-emerald-700"><CheckCircle2 size={19} />平台运行正常</div>
          )
        ) : <button onClick={handleInstallPack} disabled={installing} className="app-button-primary min-w-fit"><PackagePlus size={18} />{installing ? "正在安装..." : "安装 CRM Lite Pack"}</button>}
      </div>
    </section>
