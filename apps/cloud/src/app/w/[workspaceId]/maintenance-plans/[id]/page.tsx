...
const VIEW_KEY = "maintenance_plan_form";
...
export default function MaintenancePlanDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm("确定要删除此维保计划吗？此操作不可撤销。")) return;
    setDeleting(true);
    setError(null);
    try {
      const res = await fetch(
      ...
      if (json.success) {
        notifyWorkspaceDataChanged();
        router.push(`/w/${workspaceId}/maintenance-plans`);
        router.refresh();
      } else {
  ...
  if (!record) {
    return (
      <div className="space-y-4">
        ...
        <Link
          href={`/w/${workspaceId}/maintenance-plans`}
