...
const VIEW_KEY = "answer_block_form";

export default function AnswerBlockDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm("确定要删除此答案块吗？此操作不可撤销。")) return;
    setDeleting(true);
    setError(null);
    try {
      const res = await fetch(
      ...
      if (json.success) {
        notifyWorkspaceDataChanged();
        router.push(`/w/${workspaceId}/answer-blocks`);
        router.refresh();
      } else {
  ...
  if (!record) {
    return (
      <div className="space-y-4">
        ...
        <Link
          href={`/w/${workspaceId}/answer-blocks`}
