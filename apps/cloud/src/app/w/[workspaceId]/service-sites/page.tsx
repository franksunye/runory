...
const OBJECT_KEY = "service_site";
const VIEW_KEY = "service_site_list";
const PAGE_SIZE = 20;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">服务地点</h1>
          <p className="mt-1 text-sm text-slate-500">管理所有服务地点记录</p>
        </div>
        {hasPack && (
          <button
            type="button"
            onClick={() =>
              router.push(`/w/${workspaceId}/service-sites/new`)
            }
      ...
      {!hasPack ? (
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-8 text-center">
          ...
          <p className="mt-1 text-sm text-blue-700">安装 CRM Lite Pack 后即可开始管理服务地点。</p>
          <Link
        ...
        <div className="space-y-3">
          ...
          {totalCount === 0 ? (
            isSearching ? (
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-8 text-center">
                <p className="text-sm text-slate-500">还没有服务地点记录</p>
                <button
                  type="button"
                  onClick={() => router.push(`/w/${workspaceId}/service-sites/new`)}
