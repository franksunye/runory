...
const VIEW_KEY = "customer_success_form";
...
export default function CustomerSuccessDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm("确定要删除此客户跟进吗？此操作不可撤销。")) return;
    setDeleting(true);
    setError(null);
    try {
      const res = await fetch(
      ...
      if (json.success) {
        notifyWorkspaceDataChanged();
        router.push(`/w/${workspaceId}/customer-success`);
        router.refresh();
      } else {
  ...
  if (!record) {
    return (
      <div className="space-y-4">
        ...
        <Link
          href={`/w/${workspaceId}/customer-success`}
