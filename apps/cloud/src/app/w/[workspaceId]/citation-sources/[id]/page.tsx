...
const VIEW_KEY = "citation_source_form";

export default function CitationSourceDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm("确定要删除此引用来源吗？此操作不可撤销。")) return;
    setDeleting(true);
    setError(null);
    try {
      const res = await fetch(
      ...
      if (json.success) {
        notifyWorkspaceDataChanged();
        router.push(`/w/${workspaceId}/citation-sources`);
        router.refresh();
      } else {
  ...
  if (!record) {
    return (
      <div className="space-y-4">
        ...
        <Link
          href={`/w/${workspaceId}/citation-sources`}
