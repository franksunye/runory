...
export async function listWorkspacePeople(
  ...
  return queryAll<{ id: string; display_name: string; email: string | null }>(
...
export async function enrichUserReferences<T extends Record<string, unknown>>(
  ...
  const people = await queryAll<{ id: string; external_id: string; display_name: string; email: string | null }>(
